...
class mproxy_proxy_server:public lyramilk::netio::aioserver<lyramilk::mproxy::msock5_proxy>
{
};
...
void useage(lyramilk::data::string selfname)
{
	...
	std::cout << "\t-k <pid>\t"  << D("使指定的mproxy进程和平结束。") << std::endl;
}
...
int main(int argc,const char* argv[])
{
	...
	if(configure_file.compare(configure_file.size()-5,configure_file.size(),".json") == 0){
		...
		if(vconf.type() == lyramilk::data::var::t_map){
			lyramilk::klog(lyramilk::log::trace,"mproxy.main") << D("加载json配置文件%s成功。",configure_file.c_str()) << std::endl;
		}
		...
		if(lyramilk::data::yaml::parse(filedata + "\n...\nok\n",&ar)){
			if(ar.size() > 1 && ar[ar.size() - 1] == "ok"){
				//防止出现解析不完全的情况
				vconf = ar[0];
				lyramilk::klog(lyramilk::log::trace,"mproxy.main") << D("加载yaml配置文件%s成功。",configure_file.c_str()) << std::endl;
			}
		}
	}
	...
	if(lyramilk::data::var::t_map != vconf.type()){
		lyramilk::klog(lyramilk::log::error,"mproxy.main") << D("加载配置文件%s出现错误。",configure_file.c_str()) << std::endl;
		return -2;
	}
	if(testconfig){
		lyramilk::klog(lyramilk::log::trace,"mproxy.main") << D("测试配置文件%s未发现错误。",configure_file.c_str()) << std::endl;
		return 0;
	}
	...
	/* 己经正式决定要执行下去了。 */
	signal(SIGUSR1, mproxy_sig_leave);
	...
	if(isdaemon){
		...
		lyramilk::klog(lyramilk::log::trace,"mproxy.main") << D("以守护进程方式方式启动。",configure_file.c_str()) << std::endl;
	}else{
		enable_log_debug = true;
	}
	lyramilk::log::logss log(lyramilk::klog,"mproxy");
		...
		do{
			...
			if(pf == nullptr && operate == "trystart"){
				...
				if(pid > 0){
					...
					if(strstr(buff,"mproxyd") != nullptr){
	...
	if(isdaemon){
		do{
			chpid = fork();
			if(chpid == 0){
				lyramilk::setproctitle("mproxyd[%u]: worker process",getppid());
				break;
			}
	...
	/* 开始服务 */
	mproxy_proxy_server* ins = new mproxy_proxy_server;
	bool isok = false;
	{
		for(int i=0;i<3;++i){
			if(host.empty()){
